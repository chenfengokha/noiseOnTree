...
def runFilter(raw_line, mismatch=3):
    # define match pattern
    polyA_pattern = regex.compile("(?e)(AAA)(?P<pA>AAAAAAAAAA){e<=%d}" % mismatch)
    polyT_pattern = regex.compile("(?e)(TTT)(?P<pA>TTTTTTTTTT){e<=%d}" % mismatch)
    # fuzz match
...
if __name__ == "__main__":
    ...
    other_sam.flush()
    # create generator of raw sam file
    def gen_sam(rawSam):
        while True:
            try:
                next(rawSam)
    ...
    ## start run
    filterPool = Pool(max_workers=options.numcpu)
    #for rawline in raw_sam:
        #prf = filterPool.apply_async(runFilter, args=(rawline,), callback=mycallback)
    for each_result in filterPool.map(runFilter, gen_sam(raw_sam), chunksize= 40):
        mycallback(each_result)
    # print('waite')
    # filterPool.close()
    # filterPool.join()
    # print('done')
